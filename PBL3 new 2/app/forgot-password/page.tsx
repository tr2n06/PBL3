...
export default function ForgotPasswordPage() {
  ...
  const [resetToken, setResetToken] = useState(""); // true when countdown hits 0 after OTP sent
  ...
  try {
    ...
    setOtpError("");
    setExpired(false);
    setCountdown(res.expiresInSeconds ?? 30);
    setStep("otp");
  } catch (error) {
  ...
  try {
    ...
    setPassError(error instanceof Error ? error.message : "Failed to reset password");
  } finally {
    setLoading(false);
  }
};
