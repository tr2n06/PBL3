...
export default function CustomerBookingPage() {
  ...
  // ── Computed ──
  const handleLookupCustomerPhone = async () => {
    if (!customerPhone.trim()) {
      setLookupError("Please enter customer phone number.");
    ...
    try {
      ...
      setPointsBalance(customer.availablePoints ?? 0);
      setPointsToUseInput("0");
    } catch (error) {
      console.error("Lookup customer by phone failed:", error);
      setCustomerInfo(null);
      setPointsBalance(0);
      setPointsToUseInput("0");
      setLookupError(
  ...
  const handleSubmitInfo = (e: React.FormEvent) => {
    ...
    // Validate that at least one of CCCD or Passport is filled and format is valid
    for (let i = 0; i < passForms.length; i++) {
      const p = passForms[i];
      if (p.passengerType !== "infant") {
        const cccdVal = p.cccd?.trim() ?? "";
        const passportVal = p.passport?.trim() ?? "";

        if (!cccdVal && !passportVal) {
          alert(`Hành khách ${i + 1} (${[p.title, p.firstName, p.middleName, p.lastName].filter(Boolean).join(" ")}): Vui lòng nhập Số căn cước công dân (CCCD) hoặc Số hộ chiếu (Passport)!`);
          return;
        }

        // If passport is filled, validate its format: 1 uppercase letter + 7 digits
        if (passportVal) {
          const upperPassport = passportVal.toUpperCase();
          const passportRegex = /^[A-Z][0-9]{7}$/;
          if (!passportRegex.test(upperPassport)) {
            alert(`Hành khách ${i + 1}: Số hộ chiếu (Passport) "${passportVal}" không hợp lệ! Hộ chiếu Việt Nam hợp lệ phải gồm 1 chữ cái in hoa đứng đầu (A-Z) và 7 chữ số tiếp theo (Ví dụ: B1234567).`);
            return;
          }
        }
      }
  ...
  const handleProceedToPayment = () => {
    ...
    const pointsUsed = Math.max(
      0,
      Math.min(parsedPointsToUse, maxPointsUsable),
    );

    const total = totalWithoutDiscount - pointsUsed;

    const finalizedPassForms = passForms.map((p) => {
      // Fallback empty CCCD to Passport and ensure uppercase
      const resolvedPassport = p.passport?.trim().toUpperCase() ?? "";
      const resolvedCccd = p.cccd?.trim() ? p.cccd.trim() : resolvedPassport;
      return {
        ...p,
        cccd: resolvedCccd,
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      {passForms[idx]?.passengerType !== "infant" && (
                        <div className="space-y-3">
                          <div className="space-y-1">
                            <Label className="flex items-center gap-1 text-gray-700 font-bold">
                          ...
                          <div className="space-y-1">
                            <Label className="flex items-center gap-1 text-gray-700 font-bold">
                              Số hộ chiếu (Passport) <span className="text-xs text-gray-400 font-normal">(Không bắt buộc)</span>
                            </Label>
                            <Input
                              placeholder="Nhập số hộ chiếu"
                              value={passForms[idx]?.passport ?? ""}
                              onChange={(e) =>
                          ...
                          <p className="text-[10px] text-amber-600 font-semibold leading-normal">
                            ⚠️ Vui lòng điền ít nhất 1 trong 2 loại giấy tờ trên để thực hiện chuyến bay.
                          </p>
                        </div>
                      )}
                      {passForms[idx]?.passengerType === "child" && (
          ...
          <CardContent className="p-6 space-y-4">
            <div>
              ...
              <p className="text-sm text-yellow-700 mt-1">
                Enter customer phone number to look up reward points.
              </p>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              ...
              <Button
                type="button"
                onClick={handleLookupCustomerPhone}
                disabled={lookupLoading || !customerPhone.trim()}
                className="bg-[#f5d020] hover:bg-yellow-400 text-gray-900 font-semibold"
              >
            ...
            {customerInfo && (
              <div className="rounded-xl border border-yellow-200 bg-white/70 p-4 space-y-1">
            ...
            <div className="rounded-xl border border-yellow-200 bg-white/70 p-4 flex items-center justify-between">
              <span className="text-sm font-medium text-yellow-900">
                Available points
              </span>
              <span className="text-lg font-bold text-yellow-800">
                {formatVND(pointsBalance)}
              </span>
            </div>

            <div className="space-y-2">
              <Label className="text-yellow-900 font-medium">
                Points to use
              </Label>
              <Input
              ...
              {Number(pointsToUseInput || 0) > maxPointsUsable && (
                <p className="text-xs text-red-600">
                  Entered points exceed the allowed limit. The system will use
                  the maximum valid amount only.
                </p>
              )}
            </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {!searched ? (
        ...
        <Card className="border border-gray-200 shadow-lg bg-white rounded-3xl overflow-hidden">
          <CardHeader className="pb-2 bg-slate-50 border-b border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-lg text-gray-800 font-bold flex items-center gap-2">
                  <Search className="w-5 h-5 text-[#1a3557]" /> Modify Search
                </CardTitle>
              </div>
            </div>
          </CardHeader>
